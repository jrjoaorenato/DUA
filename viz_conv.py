...
from common.generators import PoseGenerator, PoseGeneratorSMPL
...
def parse_args():
    ...
    parser.add_argument('-dv', '--device', default='cuda:5', type=str, metavar='NAME',
...
def main(args):
    ...
    model_pos = MapPoseConverter((num_joints - 1) * 3, 1024).to(device)
    ...
    poses_smpl = dataset_smpl[args.viz_subject][args.viz_action]['positions_3d']
    out_poses_smpl = poses_smpl[args.viz_camera] #input smpl
    out_poses_3d = poses_3d[args.viz_camera]

    ground_truth = dataset[args.viz_subject][args.viz_action]['positions_3d'][args.viz_camera].copy() #ground truth

    input_keypoints = out_poses_smpl.copy()
    render_loader = DataLoader(PoseGeneratorSMPL([out_poses_3d], [out_poses_smpl], [out_actions]), batch_size=args.batch_size,
    ...
    np.save('smpl_data.npy', input_keypoints)
    ...
    input_keypoints = camera_to_world(input_keypoints, R=cam['orientation'], t=0)
    input_keypoints[:, :, 2] -= np.min(input_keypoints[:, :, 2])

    anim_output = {'SMPL': input_keypoints, 'Conversion': prediction, 'Ground truth': ground_truth}
    render_animation(anim_output, dataset.skeleton(), dataset.fps(), args.viz_bitrate, cam['azimuth'],
...
def evaluate(data_loader, model_pos, device, architecture):
    ...
    for i, (targets_3d, smpl_3d, _) in enumerate(data_loader):
        ...
        outputs_3d = model_pos(smpl_3d.view(num_poses, -1)).view(num_poses, -1, 3).cpu()
