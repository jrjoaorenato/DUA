...
from models.sem_gcn_prob import SemGCNProb, UncertaintyLoss, UncertaintyNetwork
import wandb
...
def parse_args():
    ...
    parser.add_argument('-dv', '--device', default='cuda:1', type=str, metavar='NAME',
...
def main(args):
    ...
    model_pos = SemGCNProb(adj, args.hid_dim, num_layers=args.num_layers, p_dropout=p_dropout,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Train for one epoch
        epoch_loss, epoch_loss_uncertainty, lr_now, glob_step = train(train_loader, model_pos, model_uncertainty, criterion, optimizer, device, args.lr, lr_now,
        ...
        suffix = 'uncertainty_epoch_{:04d}'.format(epoch + 1)
        ...
        if error_best is None or error_best > error_eval_p1:
            ...
            save_ckpt({'epoch': epoch + 1, 'lr': lr_now, 'step': glob_step, 'state_dict': model_uncertainty.state_dict(),
                       'optimizer': optimizer.state_dict(), 'error': error_eval_p1}, ckpt_dir_path, suffix='uncertainty_best')

        if (epoch + 1) % args.snapshot == 0:
            save_ckpt({'epoch': epoch + 1, 'lr': lr_now, 'step': glob_step, 'state_dict': model_pos.state_dict(),
                       'optimizer': optimizer.state_dict(), 'error': error_eval_p1}, ckpt_dir_path)
            save_ckpt({'epoch': epoch + 1, 'lr': lr_now, 'step': glob_step, 'state_dict': model_uncertainty.state_dict(),
                       'optimizer': optimizer.state_dict(), 'error': error_eval_p1}, ckpt_dir_path, suffix=suffix)
...
def train(data_loader, model_pos, model_uncertainty, criterion, optimizer, device, lr_init, lr_now, step, decay, gamma, max_norm=True):
...
def evaluate(data_loader, model_pos, device):
    ...
    for i, (targets_3d, inputs_2d, _) in enumerate(data_loader):
        ...
        outputs_3d, _ = model_pos(inputs_2d)
