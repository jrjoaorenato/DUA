...
def main(args):
    ...
    ckpt_dir_path = path.join(args.checkpoint, 'sur_dann', datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%f'))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Train for one epoch
        epoch_loss, lr_now, glob_step = train(train_loader, h36m_train_loader, model_pos, model_conversion,
                                              model_dann, model_uncertainty, criterion, domain_loss, optimizer, device, args.lr, lr_now,
...
def train(data_loader, h36m_train, model_pos, model_conv, model_dann, model_uncertainty, criterion, domain_loss, optimizer, device, lr_init, lr_now, epoch, step, decay, gamma, max_norm=True):
    ...
    for i, (targets_3d, inputs_2d) in enumerate(data_loader):
        ...
        dom_loss = domain_loss(outputs_3d.view(num_poses, -1), pred_h36m)
...
def evaluate(data_loader, h36m_loader, model_pos, model_conversion, device):
    ...
    for i, (surreal_3d, surreal_2d) in enumerate(data_loader):
        # Measure data loading time
        data_time.update(time.time() - end)
        num_poses_surreal = surreal_3d.size(0)

        surreal_2d = surreal_2d.to(device)
        surreal_3d = surreal_3d.to(device)
        pred_surreal, _ = model_pos(surreal_2d.view(num_poses, -1))
        pred_surreal = pred_surreal.view(num_poses, -1, 3).cpu()
        targets_3d_conv = model_conversion(surreal_3d[:,1:,:].view(num_poses, -1)).view(num_poses, -1, 3).cpu()
        targets_3d_conv = torch.cat([torch.zeros(num_poses, 1, targets_3d_conv.size(2)), targets_3d_conv], 1)  # Pad hip joint (0,0,0)
        pred_surreal = torch.cat([torch.zeros(pred_surreal, 1, outputs_3d.size(2)), pred_surreal], 1)  # Pad hip joint (0,0,0)
        
        epoch_loss_3d_pos.update(mpjpe(pred_surreal, targets_3d_conv).item() * 1000.0, num_poses)
        epoch_loss_3d_pos_procrustes.update(p_mpjpe(pred_surreal.numpy(), targets_3d_conv.numpy()).item() * 1000.0, num_poses)
