...
def render_conversion(poses, skeleton, fps, bitrate, azim, output, viewport,
                     limit=-1, downsample=1, size=6, input_video_path=None, input_video_skip=0):
    ...
    if input_video_path is None:
        # Black background
        all_frames = np.zeros((poses['SMPL'].shape[0], viewport[1], viewport[0]), dtype='uint8')
    else:
        # Load video using ffmpeg
        all_frames = []
        for f in read_video(input_video_path, skip=input_video_skip):
            all_frames.append(f)
        effective_length = min(poses['SMPL'].shape[0], len(all_frames))
        all_frames = all_frames[:effective_length]
    ...
    def update_video(i):
        ...
        if not render_animation.initialized:
            ...
            for j, j_parent in enumerate(parents):
                ...
                if len(parents) == poses['SMPL'].shape[1]:
                    # Draw skeleton only if keypoints match (otherwise we don't have the parents definition)
                    render_animation.lines.append(ax_in.plot([keypoints[i, j, 0], keypoints[i, j_parent, 0]],
                                            [keypoints[i, j, 1], keypoints[i, j_parent, 1]], color='pink'))
