...
def collect_feature(data_loader: DataLoader, feature_extractor: nn.Module,
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(tqdm.tqdm(data_loader)):
            ...
            # _, feature = feature_extractor(images, feat=True)
            feature = feature_extractor(images)
            # feature = images
            # feature = feature_extractor.autoencoder.source_encode(images)
            # feature = feature_extractor.autoencoder.decode(feature)
            # feature = feature_extractor.pose_estimator(feature)
            feature = feature.cpu()
