...
USED_SMPL_JOINTS =  [0, 2, 5, 8, 1, 4, 7,  9, 12, 15, 13, 18, 20, 14, 19, 21]
...
class SMPLDataset(MocapDataset):
    def __init__(self, path, remove_static_joints=True):
        super(SMPLDataset, self).__init__(skeleton=h36m_skeleton, fps=50)
        ...
        data = np.load(path, allow_pickle=True)['positions_3d'][()]
        ...
        for subject, actions in data.items():
            ...
            for action_name, positions in actions.items():  
                self._data[subject][action_name] = {
                    'positions': positions.squeeze()[:, USED_SMPL_JOINTS, :],
